...
def run_all_tests():
    ...
    cleaner_test_suite = unittest.TestLoader().discover("tests", pattern="test_cleaner.py")
    ...
    classifier_test_suite = unittest.TestLoader().discover("tests", pattern="test_classifier.py")
    classifier_result = unittest.TextTestRunner(verbosity=2).run(classifier_test_suite)
    results["classifier_tests"] = "Passed" if classifier_result.wasSuccessful() else "Failed"
